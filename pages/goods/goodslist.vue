<template>
	<view>
	<view class="header">
		<view @tap="handleSelect(index)" class="target" v-for="(target,index) in filteByList" :key="index" 
		...
		<view class="product-list">
			<view @tap="handleGoods(goods)" class="product" v-for="goods in goodsList" :key="goods.goods_id">
				...
				<view class="info">
					...
					<view class="slogan">{{goods.slogan}}
					<text>人付款</text>
					</view>
				</view>
			</view>
	...
	export default{
		...
		methods:{
			handleSelect(index){
				...
				// 其他所有内容都复制为false
				for(let i = 0; i<this.filteByList.length; i++){
					if(i != index){
						this.filteByList[i].selected = false;
					}
				}
				// 数据请求
				this.filterby = this.filteByList[index].filterby;
				this.page = 1;
				this.loadingText = "加载中...";
				this.goodsList = [];
				this.loadData();
